...
class TestAlgorithmByHighestOrLowest(unittest.TestCase):
    def create_chartbase(self):
        chart = Chart(span_minutes=1)
        return chart

    def test_make_position_order_alg(self):
        alg = PriceDeciderByHighestOrLowest(use_bar_count=3,
                                            open_div_rate=-0.05,
        ...
        self.assertAlmostEqual(350*0.95, p)
        ...
        (t, p) = alg.decide_make_position_order(chart)
        self.assertEqual(t, "long")
        self.assertAlmostEqual(500*0.95, p)
    ...
    def test_make_position_short_order_alg(self):
        ...
        self.assertAlmostEqual(250*1.05, p)

    def test_market_sell_decide_alg(self):
        alg = PriceDeciderByHighestOrLowest(use_bar_count=3,
                                            open_div_rate=-0.05,
                                            close_div_rate_from_buy_value=0.1,
                                            close_bar_count_to_hold=5)
        chart = self.create_chartbase()
        ...
        base_time = datetime(year=1970,month=1,day=1, tzinfo=pytz.utc) + timedelta(seconds=time_offset)
        ...
        now_time = datetime(year=1970,month=1,day=1, tzinfo=pytz.timezone('Asia/Tokyo')) + timedelta(seconds=time_offset) +  timedelta(seconds=32+60*6)
        ret = alg.market_sell_decide_algorithm(chart,
                                               105,
                                               buy_date, 
                                               now_time)
